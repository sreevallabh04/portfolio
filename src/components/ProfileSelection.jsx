...
const ProfileSelection = ({ onProfileSelect }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="flex min-h-screen flex-col items-center bg-[#121212] p-8"
    >
      <h1 className="mt-[50px] mb-16 text-center text-[48px] font-light text-white font-sans">
        Who's Watching?
      </h1>

      <div className="grid grid-cols-2 gap-[30px] md:grid-cols-4 mt-8">
        {profiles.map((profile) => (
          ...
          >
            <div className={`w-[160px] h-[160px] cursor-pointer rounded-lg shadow-lg shadow-black/50 hover:shadow-xl hover:shadow-black/60 transition-all duration-300 overflow-hidden ${!imagesLoaded[profile.id] ? profile.color : ''}`}>
            ...
            <h2 className="mt-[10px] text-center text-lg text-white font-sans">{profile.name}</h2>
          </motion.div>
        ))}
      </div>
    </motion.div>
